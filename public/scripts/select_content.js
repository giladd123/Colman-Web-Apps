document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadContent() {
    try {
      ...
      const response = await fetch(
        `/select-content/api/data/${contentId}?profileId=${encodeURIComponent(
          profileId
        )}`
      );
  ...
  function renderDetails(content, watchHabits = {}) {
    ...
    if (content.actors && content.actors.length > 0) {
      castEl.innerHTML = content.actors
        .map(
          (actor) =>
    ...
    if (content.genres && content.genres.length > 0) {
      genresEl.innerHTML = content.genres
        .map((genre) => `<a>${genre}</a>`)
        .join(", ");
    } else {
    ...
    if (content.type === "Show" && content.seasons) {
      ...
      const seasonNumbers = Object.keys(content.seasons)
        .filter((k) => content.seasons[k] && content.seasons[k].length > 0)
        .sort((a, b) => a - b);
            ...
            return `
                <div class="episode-item" data-episode-id="${episode._id}">
                    ...
                    <div class="episode-thumbnail mx-3">
                        <img src="${
                          episode.posterUrl ||
                          "https://placehold.co/300x170/222/FFF?text=No+Image"
  ...
  function attachPageListeners(
    ...
    const playFromBeginningButton = document.getElementById(
      ...
      if (contentData.type === "Movie") {
        ...
        try {
          const res = await fetch(
            `/player/api/next-episode/${showId}/${profileId}`
          );
    ...
    playFromBeginningButton.addEventListener("click", () => {
      ...
      if (contentData.type === "Movie") {
        ...
        const firstSeasonKey = Object.keys(contentData.seasons).sort(
          (a, b) => Number(a) - Number(b)
        )[0];
        if (
          firstSeasonKey &&
          contentData.seasons[firstSeasonKey] &&
          contentData.seasons[firstSeasonKey].length > 0
        ) {
    ...
    if (likeButton) {
      if (
        typeof apiAddLike !== "function" ||
        typeof apiRemoveLike !== "function" ||
        typeof animateLikeIcon !== "function"
      ) {
        console.error(
          "likes_manager.js is not loaded or is missing API/animation functions."
        );
      } else {
    ...
    if (watchlistButton) {
      if (
        typeof apiAddToWatchlist !== "function" ||
        typeof apiRemoveFromWatchlist !== "function" ||
        typeof animateWatchlistIcon !== "function"
      ) {
        console.error(
          "watchlist_manager.js is not loaded or is missing API/animation functions."
        );
      } else {
        let inList = initialIsInWatchlist;

        watchlistButton.innerHTML = inList
          ? '<i class="bi bi-check2"></i>'
          : '<i class="bi bi-plus"></i>';
        watchlistButton.title = inList
          ? "Remove from My List"
          : "Add to My List";
