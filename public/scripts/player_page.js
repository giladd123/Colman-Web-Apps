document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      video.currentTime =
        restart === "true"
          ? 0
          : data.habit
          ? data.habit.watchedTimeInSeconds
          : 0;
      document.title =
        data.content.episodeTitle || data.content.title || "Playing";
  ...
  function buildEpisodeDrawer(showData, currentEpisodeId) {
    ...
    const sortedSeasonKeys = Object.keys(showData.seasons).sort(
      ...
      for (const episode of episodes) {
        ...
        item.innerHTML = `
          ...
          <div class="episode-item-info">
            <div class="episode-item-title">${episode.episodeNumber}. ${
  ...
  async function saveProgress(isComplete = false) {
    ...
    if (!contentId || !profileId || (video.currentTime === 0 && !isComplete))
      return;
