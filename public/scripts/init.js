...
async function initializeGenresDropdown() {
  try {
    ...
    if (dropdownMenu && genres.length > 0) {
      ...
      dropdownMenu.innerHTML = '<li><a class="dropdown-item text-white" href="#">No genres available</a></li>';
    }
  } catch (error) {
    console.error('Error fetching genres:', error);
    const dropdownMenu = document.getElementById('genresDropdownMenu');
    if (dropdownMenu) {
      dropdownMenu.innerHTML = '<li><a class="dropdown-item text-white" href="#">Error loading genres</a></li>';
    }
  }
}
