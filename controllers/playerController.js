...
async function findShowAndNextEpisode(episode, showId) {
  try {
    ...
    if (!showId) {
      warn(
        `Attempted to find show for episode ${contentId} but no showId was provided in query.`
      );
    ...
    for (const key of sortedSeasonKeys) {
      ...
      const index = episodes.findIndex(
        (ep) => String(ep._id) === String(contentId)
      );
      if (index > -1) {
    ...
    logError(`Error in findShowAndNextEpisode: ${err.message}`, {
      stack: err.stack,
    });
    return { showData: null, nextEpisodeId: null };
  }
...
export async function getNextEpisodeToPlay(req, res) {
  ...
  if (habits.length > 0) {
    ...
    const { nextEpisodeId } = await findShowAndNextEpisode(
      await Episode.findById(lastHabit.contentId),
      showId
    );
    if (nextEpisodeId) {
  ...
  const firstSeasonKey = Array.from(show.seasons.keys()).sort(
