...
export const getContentDataById = async (req, res) => {
  ...
  if (!selectedContent) {
    warn(
      `Content with ID ${contentId} not found for user ${
        profileName || "unknown"
      }`
    );
  ...
  if (!selectedContent.type) {
    logError(
      `Data integrity error: Content ${contentId} is missing the 'type' field.`,
  ...
  if (profile) {
    ...
    isLiked =
      profile.likedContents?.some((id) => id.equals(contentId)) || false;
    isInWatchlist =
      profile.watchlist?.some((id) => id.equals(contentId)) || false;
    ...
    if (selectedContent.type === "Movie") {
      ...
      const allEpisodeIds = Array.from(selectedContent.seasons.values())
        .flat()
        .map((ep) => ep._id || ep);
      ...
      const seasonKeys = Array.from(selectedContent.seasons.keys());
      const lastSeasonKey = seasonKeys
      ...
      if (lastSeason && lastSeason.length > 0) {
        ...
        const lastEpisodeHabit = habits.find((h) =>
          h.contentId.equals(lastEpisodeId)
        );
